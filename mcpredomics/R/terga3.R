...
#' @export
terga1_ova <- function(sparsity = c(1:10),
                   # population options
                   ...
                   experiment.id = "NULL", experiment.description = "NULL", experiment.save = "nothing")
{
  ...
  clf$learner <- "terga1_ova" # name of the method
...
terga1_ova_fit <- function(X, y, clf) {
  ...
  for(i in clf$params$sparsity) # sparsity is = k, i.e. the number of features in a model
  {
   ...
   # for(u in 1: length(unique_y)){
     # y_ova <- ifelse(y== unique_y[u], 1,-1)
     # ova[[u]] <- y_ova
    #}

   # for(u in 1:length(ova)){

      pop_last.mod <- listOfSparseVecToListOfModels(X = X, y, clf = clf, v = pop_last)
...
#' @export
evolve_ova <- function(X, y, clf, pop, seed = NULL)
{
  unique_y <- unique(y)
  ...
  for(i in 1:length(ova)){
    list_evolved_pop[[i]] <- evolve(X, y=ova[[i]], clf, pop, seed = NULL)
