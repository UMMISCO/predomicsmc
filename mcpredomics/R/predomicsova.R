                    ...
                    path = NULL)
{
  ...
  }else
  {
    ...
    if(clf$params$objective == "cor") # correlation
    {
      ...
      feature.cor     <- filterfeaturesK_ova(data = t(apply(X.nona, 1, rank)), # for speedup
    ...
    {
      feature.cor     <- filterfeaturesK_ova(data = X,
                                             trait = y,
                                             k = max.nb.features,
                                             type = "wilcoxon",
                                             sort = TRUE,
  ...
  for (j in 1:length(feature.cor)){
    ...
    y_ova <- ifelse(y== unique_y[j], 1,-1)
    ova[[j]] <- y_ova
    cat("... Storing data in the classifier object for speedup\n")
    clf$feature.cor <- feature.cor[[j]] # add them to the clf
    cat("... Storing data in the classifier object for speedup\n")
    #clf$feature.cor[[i]] <- feature.cor[[i]] # add them to the clf
    ...
    #clf$data$features[[i]] <- as.character(rownames(X))
    ...
    clf$data$X <- X[rownames(clf$feature.cor)[1:max.nb.features],]

    clf$data$X.min    <- min(clf$data$X, na.rm = TRUE)
    clf$data$X.max    <- max(clf$data$X, na.rm = TRUE)
    clf$data$y       <- y
    # compute the coefficients once for all to improve performance
    cat("... Computing ternary coefficients for speedup\n")
    coeffs          <- getSign(X = X, y = ova[[j]], clf = clf, parallel.local = FALSE)
    ...
    if(!cross.validate)
    {
      ...
      res.clf               <- list()
      res.clf$classifier    <- runClassifier_ova(X, y= ova[[j]], clf)
    ...
    } else
    {
      ...
      res.clf$crossVal      <- runCrossval_ova(X, y= ova[[j]], clf, lfolds = lfolds, nfolds = nfolds, return.all = return.all)
  ...
  list_res.clf[[j]] <- res.clf
  }
  res.clf <- list_res.clf
  return(res.clf)
}
...
runClassifier_ova <- function(X, y, clf, x_test = NULL, y_test = NULL)
{
...
#' @export
runCrossval_ova <- function(X, y, clf, lfolds = NULL, nfolds = 10, return.all = FALSE)
{
    ...
    res.all <- foreach(i = 1:nfolds) %dorng%
      {
        ...
        } else
        {
          ...
          if(clf$params$debug)
          {
            ...
            runClassifier_ova(X = x_train, y =  y_train, clf = clf, x_test = x_test, y_test = y_test)
          }else
          {
            try(
              {
  ...
  }else # no parallel
  {
    ...
    for (i in 1:nfolds)
    {
      ...
      if(clf$params$debug)
      {
        res.all[[i]]                    <- runClassifier_ova(X = x_train, y =  y_train, clf = clf, x_test = x_test, y_test = y_test)
      }else
      {
        res.all[[i]]                    <- try(
          {
  ...
  for(i in 1:length(res.all))
  {
    ...
    if(!is.null(res_train.digest))
    {
      ...
      for(k in 1:length(res_train.digest$best_models))
      {
        ...
        mod.test          <- evaluateModel_ova(mod = mod, X=x_test, y=y_test, clf=clf, eval.all = TRUE, force.re.evaluation = TRUE, mode='test')
