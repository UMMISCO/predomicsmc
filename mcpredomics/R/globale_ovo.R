...
#'
#' @description This function evaluates the predicted classification either using (1) a model object that contains intercept and sign or (2) directly the attributes score, intercept, sign. one versus one
...
evaluateYhat_ovo <- function(mod = NULL, X, y, clf, score=NULL, intercept=NULL, sign=NULL)
{
  ...
  for(i in 1:length(list_y)){
    yha <- evaluateYhat(mod = NULL, X=list_X[[i]], y=list_y[[i]], clf, score=NULL, intercept=NULL, sign=NULL)
    yhat_list[[i]] = yha
  }
  yhat = yhat_list
...
' One Versus One, Computes the AUC of a model
#'
#' @description One Versus one, Computes the AUC of a model
...
evaluateIntercept_ovo <- function(mod, X, y, clf)
{
  ...
  for(i in 1:length(list_y)){
    eva <- evaluateIntercept(mod, X=list_X[[i]], y=list_y[[i]], clf)
...
#'
#' @import foreach
#' @title evaluatePopulation_ovo
#' @name evaluatePopulation_ovo
#' @param X: the data matrix with variables in the rows and observations in the columns
                                   ...
                                   lfolds = NULL)
{
  ...
  # list evaluate population one versus one
  for(i in 1:length(list_y)){
    evaPop <- evaluatePopulation(X=list_X[[i]], y=list_y[[i]], clf, pop, eval.all = FALSE,
                                 force.re.evaluation = FALSE,
                                 estim.feat.importance = FALSE,
                                 mode = "train",
                                 delete.null.models = TRUE,
...
evaluateModel_ovo <- function(mod, X, y, clf, eval.all = FALSE, force.re.evaluation = FALSE, estim.feat.importance = FALSE, mode = 'train')
{
  ...
  for(i in 1:length(list_y)){
    evamodel <- evaluateModel(mod, X=list_X[[i]], y=list_y[[i]], clf, eval.all = FALSE, force.re.evaluation = FALSE, estim.feat.importance = FALSE, mode = 'train')
...
evaluateFit_ovo <- function(mod, X, y, clf, force.re.evaluation = FALSE, mode = "train")
{
  ...
  # list evaluate fit one versus one
  for(i in 1:length(list_y)){
    evafit <-  evaluateFit(mod, X=list_X[[i]], y=list_y[[i]], clf, force.re.evaluation = FALSE, mode = "train")
    list_Fit[[i]] = evafit
  }
  mod <- list_Fit
...
#' One Versus One, Evaluates the accuracy of a model one versus all
#'
#' @description One Versus one, This function evaluates the accuracy of either (1) a model object one versus all that contains intercept and sign or (2) directly the attributes score, intercept, sign
#' @param mod: a model object to be used in the class prediction
#' @param X: dataset to classify
#' @param y: variable to predict
#' @param clf: an object containing the different parameters of the classifier
...
evaluateAccuracy_ovo <- function(mod = NULL, X, y, clf, force.re.evaluation = FALSE, mode = "train")
{
  ...
  for(i in 1:length(list_y)){
    mod_ <- evaluateAccuracy(mod = NULL, X=list_X[[i]], y=list_y[[i]], clf, force.re.evaluation = FALSE, mode = "train")
...
#' One Versus One, Compute other prediction scores such as precision, recall and f-score a model
#'
#' @description One Versus one, This function computes prediction scores based on the confusion matrix such as accuracy, precision, recall and f-score a model one versus all
...
evaluateAdditionnalMetrics_ovo <- function(mod, X, y, clf, mode = "train")
{
  ...
  for(i in 1:length(list_y)){
    mod_ <- evaluateAdditionnalMetrics_ova(mod, X=list_X[[i]], y=list_y[[i]], clf, mode = "train")
...
#' evaluates the feature importance in a population of models one versus one
#'
#' @description One Versus one, This function perturbes the dataset by shuffling one at a time a subset of features that appear in a population of models
...
#' @export
evaluateFeatureImportanceInPopulation_ovo <- function(pop, X, y, clf, score = "fit_", filter.ci = TRUE, method = "optimized", seed = c(1:10), aggregation = "mean", verbose = TRUE)
{
  nClasse <- unique(y)
  yhat_list <- list()
  list_y <- list()
  ...
  #evaluate feature importance in population each model one versus one
  for(i in 1:length(list_y)){
    res_ <- evaluateFeatureImportanceInPopulation(pop=pop, X=list_X[[i]], y=list_y[[i]], clf, score = "fit_",
