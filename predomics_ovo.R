                    ...
                    path = NULL)
{
  ...
  for (i in 1:(length(list_y))) {
      ...
      if(!is.matrix(list_X[[i]]))
      {
        ...
        X <- as.matrix(as.data.frame(list_X[[i]]))
      }

      # if no rownames add some
      if(is.null(rownames(list_X[[i]])))
      {
        rownames(X) <- paste("F", c(1:nrow(list_X[[i]])), sep="_")
        cat("... No names for X variables: renaming by default\n")
      }

      if(ncol(list_X[[i]]) != length(list_y[[i]]))
      {
      ...
      if(clf$params$objective == "auc")
      {
        ...
        y <- factor(as.character(list_y[[i]]))
      }

      # The possibility to select and focus on the top best features of X
      if(!is.null(clf$params$max.nb.features))
      {
        max.nb.features <-  min(nrow(list_X[[i]]), clf$params$max.nb.features, na.rm = TRUE)
      }else
      {
        max.nb.features <-  nrow(list_X[[i]])
      }
      ...
      }else
      {
        ...
        {
          feature.cor     <- filterfeaturesK(data = list_X[[i]],
                                             trait = list_y[[i]],
                                             k = max.nb.features,
                                             type = "wilcoxon",
                                             sort = TRUE,
      ...
      clf$data$X        <- list_X[[i]]
      clf$data$X.min    <- min(list_X[[i]], na.rm = TRUE)
      clf$data$X.max    <- max(list_X[[i]], na.rm = TRUE)
      clf$data$y        <- list_y[[i]]

      # compute the coefficients once for all to improve performance
      cat("... Computing ternary coefficients for speedup\n")
      coeffs          <- getSign(X = list_X[[i]], y = list_y[[i]], clf = clf, parallel.local = FALSE)
      clf$coeffs_     <- coeffs # add them to the clf


      # check sparsity not to be larger than variables in X
      if(any(is.na(clf$params$sparsity > nrow(list_X[[i]]))))
      {
        # adding the maximum number of featuers
        clf$params$sparsity <- c(clf$params$sparsity, nrow(list_X[[i]]))
      }
      # mark NAs the bigger ones
      clf$params$sparsity[clf$params$sparsity > nrow(list_X[[i]])] <- NA
      # delete them
      ...
      if(clf$experiment$save != "nothing")
      {
        ...
        setwd(fileNames)
        saveResults(list_X[[i]], paste("X", fileNames, "yml", sep = "."))
        saveResults(list_y[[i]], paste("Y", fileNames, "yml", sep = "."))
        experiment <- list()
      ...
      if(!cross.validate)
      {
        ...
        res.clf               <- list()
        res.clf$classifier    <- runClassifier(list_X[[i]], list_y[[i]], clf)
      ...
      } else
      {
        ...
        res.clf$crossVal      <- runCrossval(list_X[[i]], list_y[[i]], clf, lfolds = lfolds, nfolds = nfolds, return.all = return.all)
  ...
  return(res.clf)
    ...
    runClassifier <- function(X, y, clf, x_test = NULL, y_test = NULL)
    {
      ...
      if(clf$params$compute.importance & !isLearnerSota(clf))
      {
        ...
        efip.fold <- evaluateFeatureImportanceInPopulation(pop = pop.fold,
                                                           X = x_test,
                                                           y = y_test,
                                                           clf = clf,
                                                           score = "fit_",
    ...
    #' @export
    runCrossval <- function(X, y, clf, lfolds = NULL, nfolds = 10, return.all = FALSE)
    {
      ...
      } else
      {
        ...
        if(length(clf$params$seed)>1)
        {
          ...
          {
            lfolds.tmp      <- create.folds(y[!is.na(y)], k = nfolds, list = TRUE, returnTrain = FALSE, seed = clf$params$seed[i]) # Index of 10-fold
            names(lfolds.tmp) <- paste(names(lfolds.tmp),"_r",i,sep="") # modify names
            lfolds          <- c(lfolds, lfolds.tmp)
          }
        }else # if only once
        {
          lfolds            <- create.folds(y[!is.na(y)], k = nfolds, list = TRUE, returnTrain = FALSE, seed = clf$params$seed) # Index of 10-fold
        }
      ...
      }else # no parallel
      {
        ...
        for (i in 1:nfolds)
        {
          ...
          if(clf$params$debug)
          {
            res.all[[i]]                    <- runClassifier(X = x_train, y =  y_train, clf = clf, x_test = x_test, y_test = y_test)
          }else
          {
            res.all[[i]]                    <- try(
              {
      ...
      for(i in 1:length(res.all))
      {
        ...
        if(!is.null(res_train.digest))
        {
          ...
          for(k in 1:length(res_train.digest$best_models))
          {
            ...
            mod.test          <- evaluateModel(mod = mod, X=x_test, y=y_test, clf=clf, eval.all = TRUE, force.re.evaluation = TRUE, mode='test')
