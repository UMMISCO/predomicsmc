...
classes_list <- predict_ova(mod, class_names)
...
```{r}
aggregate_predictions <- function(classes_list, score_list, list_intercept) {
  ...
  for (i in seq_along(aggregated_predictions)) {
    ...
    # Check if all classes are 'ALL'
    if (all(current_classes == "ALL")) {
      # If all classes are 'ALL,' choose the one with the highest score.
      max_score_index <- which.max(scores)
      predicted_class <- unique(y[max_score_index])
    } else {
      # If 'ALL' is not the only class, choose the class with the highest score.
      ...
      if (length(non_all_classes) > 0) {
        ...
        # If no non-'ALL' classes, choose either 'ALL' or another class with the highest score.
        max_score_index <- which.max(scores)
        predicted_class <- unique(y[max_score_index])
      }
    }
