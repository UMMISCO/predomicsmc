...
importance_scores <- as.data.frame(importance(rf))
importance_scores$Variable <- rownames(importance_scores)
importance_scores <- importance_scores %>%
  select(Variable, MeanDecreaseAccuracy)  # Select the appropriate importance measure

importance_scores <- importance_scores %>%
  arrange(desc(MeanDecreaseAccuracy))  # Sort by importance scores

# Create the Variable Importance Plot with ggplot2
ggplot(importance_scores, aes(x = reorder(Variable, MeanDecreaseAccuracy), y = MeanDecreaseAccuracy)) +
  geom_bar(stat = "identity", fill = "blue") +
  labs(
    title = "Variable Importance Plot (Multiclass)",
    x = "Variable",
...
# Exemple de données (vous pouvez utiliser vos propres données)
set.seed(123)
data(iris)
...
feature_importance <- importance(model, type = 2)

# Créer un dataframe pour les données d'importance
...
# Tracé de la courbe d'importance des fonctionnalités avec ggplot2
ggplot(importance_df, aes(x = reorder(Feature, -Importance), y = Importance)) +
  geom_bar(stat = "identity", fill = "blue") +
  coord_flip() +
  labs(title = "Importance des Fonctionnalités", x = "Fonctionnalité", y = "Importance")
