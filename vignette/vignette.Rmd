...
if(runit)
{
tMulti_Predomics_aggregation_ovo <- fit_mc(X = X, y = y, clf = clf,approch="ovo", cross.validate = TRUE,aggregation_ = "Predomics_aggregation_ovo", nfolds= 1); 
  #save(Multi_Predomics_aggregation_ovo , clf, file ="Multi_Predomics_aggregation_ovo.rda", compression_level = 9)
}
...
```{r digesting results, fig.height=8, fig.width=6, message=FALSE, warning=FALSE, paged.print=FALSE}
res_clf.dig_multi <- digestmc(obj = Multi_Predomics_aggregation_ovo , penalty = 0.75/100, plot = TRUE)
```
...
plots11 <- plotModel_mc(best.model_multi, X, y, sort.features = FALSE, feature.name = TRUE, approch = "ovo")
plots22 <- plotModel_mc(best.model_multi, X, y, sort.features = FALSE, feature.name = TRUE, importance = TRUE, approch = "ovo")
