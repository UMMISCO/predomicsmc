...
clf_Multi_weightedAggregation <- digestmc(obj = Multi_weightedAggregation   , penalty = 0.75/100, plot = TRUE)

```


```{r best model test, warning=FALSE}

bestsModelsTest_votingAggregation <- evaluatesBestsModelsTest(experiences = Multi_Predomics_Aggregation_ova, approch = "ova", aggregation_ = "Predomics_aggregation_ova", clf =clf, y = y, X = X)
...
```{r}
printy_mc(clf_Multi_weightedAggregation$best$model)
```

```{r}
  mod_testi <- evaluateModel_mc(
    mod = clf_Multi_weightedAggregation$best$model,
    X = X.test,
    y = y.test,
    clf = clf,
    eval.all = TRUE,
    ...
    approch = "ovo",
    aggregation_ = "weightedAggregation",
    mode = "test"
  )
```
...
```{r}
##Proposez moi un plot avec ggplot qui me represente dans un graphique mes trois accuracy sachant que la liste 1 doit etre nommé comme k_2, la liste 2 K_3 et enfin la liste 3 k_4 : bestsModelsTest_votingAggregation[[1]]$accuracy_
##[1] 0.6287879
##> bestsModelsTest_votingAggregation[[2]]$accuracy_
##[1] 0.6666667
##> bestsModelsTest_votingAggregation[[3]]$accuracy_
##[1] 0.6742424 Faites moi un graphique sur R avec ggplot qui va me présenter les accuracy de me trois valeur que voici
```
