...
if(runit)
{
Uue_maxi <- fit_mc(X = X, y = y, clf = clf,approch="ovo", cross.validate = TRUE,aggregation_ = "weightedAggregation", nfolds= 1); # class(res_clf_ovo)
  # save results
...
clf_ovo_we <- digestmc(obj = Uue_maxi , penalty = 0.75/100, plot = TRUE)
...
if(runit)
{
terga1mc_maximizationAggregation <- fit_mc(X = X, y = y, clf = clf,approch="ova", aggregation_ = "maximizationAggregation", cross.validate = TRUE, nfolds= 1); # class(res_clf_ovo)
  # save results
  save(terga1mc_maximizationAggregation  , clf, file = "terga1mc_maximizationAggregation.rda", compression_level = 9)
}
...
```{r digesting results, fig.height=8, fig.width=6, message=FALSE, warning=FALSE, paged.print=FALSE}
clf_ovo_weightedAggre <- digestmc(obj =terga1mc_maximizationAggregation   , penalty = 0.75/100, plot = TRUE)
```
...
```{r}
pop2 = res_clf_ovo_$classifier$models
Voting_Aggregation_FBM_11 <- evaluatePopulation_aggregation(pop = pop1, y = y.test, X = X.test, force.re.evaluation = TRUE, clf,  aggregation = "votingAggregation")
# Sauvegarder l'objet sous forme d'un fichier .rda
#save(Voting_Aggregation_FBM_1, file = "Voting_Aggregation_FBM_1.rda")
...
```{r}
load("res_clf_ovo_votingAggregation.rda")
```
...
data <- data.frame(
  ...
  Value = c(
    extract_metric_values(Voting_Aggregation_FBM_11, "accuracy_") * 100,
    extract_metric_values(Voting_Aggregation_FBM_11, "precision_") * 100,
    extract_metric_values(Voting_Aggregation_FBM_11, "recall_") * 100,
    extract_metric_values(Voting_Aggregation_FBM_11, "f1_") * 100
  ),
...
```{r}
Voting_Aggregation_FBM_ = Voting_Aggregation_FBM_11[[2]][[1]]
Voting_Aggregation_FBM_$accuracy_
Voting_Aggregation_FBM_$confusionMatrix_
Voting_Aggregation_FBM_$coeffs_
Voting_Aggregation_FBM_$predictions_aggre


```

```{r}
accuracy_val = Voting_Aggregation_FBM_$accuracy_
precision_val = Voting_Aggregation_FBM_$precision_
recall_val = Voting_Aggregation_FBM_$recall_
f1_val <- Voting_Aggregation_FBM_$f1_

# Création du dataframe sans la matrice de confusion
df_voting1 <- data.frame(
...
```{r}
pop3 = res_clf_ovo_$classifier$models
Weighted_Aggregation_FBM_22 <- evaluatePopulation_aggregation(pop = pop3, y = y.test, X = X.test, force.re.evaluation = TRUE, clf,  aggregation = "weightedAggregation")
# Sauvegarder l'objet sous forme d'un fichier .rda
#save(Weighted_Aggregation_FBM_2, file = "Weighted_Aggregation_FBM_2.rda")
```



```{r}
load("Weighted_Aggregation_FBM_2.rda")
```
...
data <- data.frame(
  Metric = rep(c("Accuracy", "Precision", "Recall", "F1"), each = 4),
  Value = c(
    extract_metric_values(Weighted_Aggregation_FBM_22, "accuracy_") * 100,
    extract_metric_values(Weighted_Aggregation_FBM_22, "precision_") * 100,
    extract_metric_values(Weighted_Aggregation_FBM_22, "recall_") * 100,
    extract_metric_values(Weighted_Aggregation_FBM_22, "f1_") * 100
  ),
  Method = rep(c("K_2", "K_3", "K_4", "K_5"), times = 4)
)
...
```{r}
Weighted_Aggregation_FBM_ = Weighted_Aggregation_FBM_22[[2]][[1]]
Weighted_Aggregation_FBM_$accuracy_
Weighted_Aggregation_FBM_$confusionMatrix_
Weighted_Aggregation_FBM_$coeffs_
Weighted_Aggregation_FBM_$predictions_aggre

```

```{r}
printy_mc(Weighted_Aggregation_FBM_)
```

```{r}
accuracy_val = Weighted_Aggregation_FBM_$accuracy_
precision_val = Weighted_Aggregation_FBM_$precision_
recall_val =Weighted_Aggregation_FBM_$recall_
f1_val <- Weighted_Aggregation_FBM_$f1_

# Création du dataframe sans la matrice de confusion
df_weighted1 <- data.frame(
...
```{r digesting results, fig.height=8, fig.width=6, message=FALSE, warning=FALSE, paged.print=FALSE}
bests_models_parsimony_ova <- digestmc(obj = res_clf_ova_, penalty = 0.75/100, plot = TRUE)
```
...
```{r}
pop1 = res_clf_ova_$classifier$models
NewApproach_Aggregation_FBM11 <- evaluatePopulation_aggregation(pop = pop, y = y.test, X = X.test, force.re.evaluation = TRUE, clf,  aggregation = "NewApproach")
# Sauvegarder l'objet sous forme d'un fichier .rda
#save(NewApproach_Aggregation_FBM1, file = "NewApproach_Aggregation_FBM1.rda")
```



```{r}
load("NewApproach_Aggregation_FBM1.rda")
```
...
data <- data.frame(
  Metric = rep(c("Accuracy", "Precision", "Recall", "F1"), each = 4),
  Value = c(
    extract_metric_values(NewApproach_Aggregation_FBM11, "accuracy_") * 100,
    extract_metric_values(NewApproach_Aggregation_FBM11, "precision_") * 100,
    extract_metric_values(NewApproach_Aggregation_FBM11, "recall_") * 100,
    extract_metric_values(NewApproach_Aggregation_FBM11, "f1_") * 100
  ),
  Method = rep(c("K_2", "K_3", "K_4", "K_5"), times = 4)
)
...
```{r}
NewApproach_Aggregation_FBM_ = NewApproach_Aggregation_FBM11[[4]][[1]]
NewApproach_Aggregation_FBM_$accuracy_
NewApproach_Aggregation_FBM_$confusionMatrix_
NewApproach_Aggregation_FBM_$coeffs_
NewApproach_Aggregation_FBM_$predictions_aggre
```


```{r}
predomics_aggregation_ova1 = NewApproach_Aggregation_FBM_
```

```{r}
accuracy_val = predomics_aggregation_ova1$accuracy_
precision_val = predomics_aggregation_ova1$precision_
recall_val =predomics_aggregation_ova1$recall_
f1_val <- predomics_aggregation_ova1$f1_

# Création du dataframe sans la matrice de confusion
df_predomics_aggregation_ova1 <- data.frame(
  Algorithme = "predomics_aggregation_ova",
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(accuracy_val, precision_val, recall_val, f1_val)
)
...
```{r}
pop1 = res_clf_ova_$classifier$models
Maximization_Aggregation_FBM11 <- evaluatePopulation_aggregation(pop = pop1, y = y.test, X = X.test, force.re.evaluation = TRUE, clf,  aggregation = "maximizationAggregation")
# Sauvegarder l'objet sous forme d'un fichier .rda
#save(Maximization_Aggregation_FBM1, file = "Maximization_Aggregation_FBM1.rda")
```


```{r}
load("Maximization_Aggregation_FBM1.rda")
```
...
data <- data.frame(
  Metric = rep(c("Accuracy", "Precision", "Recall", "F1"), each = 4),
  Value = c(
    extract_metric_values(Maximization_Aggregation_FBM1, "accuracy_") * 100,
    extract_metric_values(Maximization_Aggregation_FBM1, "precision_") * 100,
    extract_metric_values(Maximization_Aggregation_FBM1, "recall_") * 100,
    extract_metric_values(Maximization_Aggregation_FBM1, "f1_") * 100
  ),
  Method = rep(c("K_2", "K_3", "K_4", "K_5"), times = 4)
)
...
```{r}
Maximization_Aggregation_FBM_ = Maximization_Aggregation_FBM1[[4]][[1]]
Maximization_Aggregation_FBM_$accuracy_
Maximization_Aggregation_FBM_$confusionMatrix_
Maximization_Aggregation_FBM_$coeffs_
Maximization_Aggregation_FBM_$predictions_aggre
```
 
```{r}
printy_mc(Maximization_Aggregation_FBM_)
```
```{r}
accuracy_val = Maximization_Aggregation_FBM_$accuracy_
precision_val = Maximization_Aggregation_FBM_$precision_
recall_val = Maximization_Aggregation_FBM_$recall_
f1_val <- Maximization_Aggregation_FBM_$f1_

# Création du dataframe sans la matrice de confusion
df_maximization1 <- data.frame(
...
```{r warning=FALSE}
pop1 = res_clf_ova_$classifier$models
Ranking_Aggregation_FBM1 <- evaluatePopulation_aggregation(pop = pop1, y = y.test, X = X.test, force.re.evaluation = TRUE, clf,  aggregation = "rankingAggregation")
# Sauvegarder l'objet sous forme d'un fichier .rda
#save(Ranking_Aggregation_FBM, file = "Ranking_Aggregation_FBM.rda")
```


```{r}
load("Ranking_Aggregation_FBM.rda")
```
...
data <- data.frame(
  Metric = rep(c("Accuracy", "Precision", "Recall", "F1"), each = 4),
  Value = c(
    extract_metric_values(Ranking_Aggregation_FBM1, "accuracy_") * 100,
    extract_metric_values(Ranking_Aggregation_FBM1, "precision_") * 100,
    extract_metric_values(Ranking_Aggregation_FBM1, "recall_") * 100,
    extract_metric_values(Ranking_Aggregation_FBM1, "f1_") * 100
  ),
  Method = rep(c("K_2", "K_3", "K_4", "K_5"), times = 4)
)
...
```{r}
Ranking_Aggregation_FBM_ = Ranking_Aggregation_FBM1[[4]][[1]]
Ranking_Aggregation_FBM_$accuracy_
Ranking_Aggregation_FBM_$confusionMatrix_
Ranking_Aggregation_FBM_$coeffs_
Ranking_Aggregation_FBM_$predictions_aggre
```
...
```{r}
accuracy_val = Ranking_Aggregation_FBM_$accuracy_
precision_val =Ranking_Aggregation_FBM_$precision_
recall_val = Ranking_Aggregation_FBM_$recall_
f1_val <-Ranking_Aggregation_FBM_$f1_

# Création du dataframe sans la matrice de confusion
df_ranking1 <- data.frame(
...
```{r}
df_all1 <- rbind(df_ranking1, df_maximization1,df_predomics_aggregation_ova1, df_voting1,df_weighted1)
...
for (metric in metrics_list) {
  ...
  df_metric <- df_all1[df_all1$Metrics == metric, ]
...
df_all <- rbind(df_ranking1, df_maximization1,df_predomics_aggregation_ova1, df_voting1,df_weighted1, df_rf,df_LR, df_SVM)
...
```{r}
# Supposons que votre data frame soit stocké dans res_clf_ovo_$crossVal$scores$empirical.auc
df <- res_clf_ovo_$crossVal$scores$generalization.auc

# Spécifiez le chemin et le nom du fichier Excel à créer
file_path <- "~/Documents/auc_generalise.xlsx"

# Exporter le data frame en fichier Excel
...
df_ranking1 <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.5606061, 0.6953305, 0.5606061, 0.6207423)
)
...
df_predomics_aggregation_ova1 <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.6287879, 0.6465446, 0.6287879, 0.6375426)
)

# Combiner les dataframes
combined_df <- bind_rows(df_ranking1, df_maximization1, df_predomics_aggregation_ova1)

# Créer le graphique
ggplot(combined_df, aes(x = Metrics, y = Values, fill = Algorithme)) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_wrap(~ Algorithme) +
  theme_minimal() +
  labs(title = "Performance des Algorithmes selon Différentes Métriques",
       x = "Métriques",
       y = "Valeurs",
       fill = "Algorithme") +
...
```{r warning=FALSE, fig.height=5, fig.width=8, message=FALSE}
# Charger les packages nécessaires
library(ggplot2)
library(dplyr)
...
combined_df <- bind_rows(
  df_ranking1, df_maximization1, df_predomics_aggregation_ova1, df_voting1,
  df_weighted1, df_rf, df_LR, df_SVM
)

# Créer le graphique avec les valeurs au-dessus des barres
ggplot(combined_df, aes(x = Algorithme, y = Values, fill = Algorithme)) +
  geom_bar(stat = "identity", position = "dodge") +
  geom_text(aes(label = round(Values, 2)), vjust = -0.5, position = position_dodge(width = 0.9), size = 3) +
...
```{r}
# Vos dataframes (remplacer avec vos données réelles)
df_ranking1 <- data.frame(
  Algorithme = "ranking aggregation",
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
...
df_voting1 <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.5806061, 0.6853305, 0.5806061, 0.6307423)
)
...
df_weighted1 <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.5906061, 0.6753305, 0.5906061, 0.6407423)
)
...
df_rf <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.6006061, 0.6653305, 0.6006061, 0.6507423)
)
...
df_LR <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.6106061, 0.6553305, 0.6106061, 0.6607423)
)
...
df_SVM <- data.frame(
  ...
  Metrics = c("Accuracy", "Precision", "Recall", "F1"),
  Dataset = "Test",
  Values = c(0.6206061, 0.6453305, 0.6206061, 0.6707423)
)
...
```{r warning=FALSE, fig.height=2.5, fig.width=6, message=FALSE}
v2 = printy_mc(pop1$k_2[[1]])
v3 = printy_mc(pop1$k_3[[1]])
v4 = printy_mc(pop1$k_4[[1]])
v5 = printy_mc(pop1$k_5[[1]])
```
```{r}
#v2 
#v3 
...
# Transformer les listes en data frames
df_list <- lapply(names(pop11), function(k) {
  data.frame(Model = unlist(pop11[[k]]), Group = k, stringsAsFactors = FALSE)
})

# Combiner les data frames en un seul
...
# Définir une fonction pour créer un graphique ggplot à partir d'une liste de données
create_plot <- function(data, title) {
  ggplot(data, aes(x = 1, y = Model, label = Model)) +
    geom_point(size = 3, color = "blue") +
    theme_void() +
    theme(axis.text.y = element_text(size = 12)) +
    ggtitle(title) +
    geom_text(size = 3, hjust = 0, vjust = 0.5, color = "black")
}

# Créer les graphiques individuels
plot_v2 <- create_plot(data.frame(Model = unlist(v2)), "K=2")
plot_v3 <- create_plot(data.frame(Model = unlist(v3)), "K=3")
plot_v4 <- create_plot(data.frame(Model = unlist(v4)), "K=4")
plot_v5 <- create_plot(data.frame(Model = unlist(v5)), "K=5")

# Afficher les graphiques côte à côte
grid.arrange(plot_v2, plot_v3, plot_v4, plot_v5, ncol = 1)


```

```{r}
num_outer <- length(res)
num_inner <- length(res[[1]])

# Initialiser une nouvelle liste vide pour stocker les résultats
res_inverted <- vector("list", num_inner)

# Boucler sur les indices intérieurs et extérieurs pour inverser la structure
for (j in 1:num_inner) {
  res_inverted[[j]] <- lapply(res, function(x) x[[j]])
}
