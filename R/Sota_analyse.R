...
#
# # =========================================================
# # RANDOM FOREST OvO + MAJORITY VOTING WITH TIE-BREAKING
# # 10-fold cross-validation with full object storage
# # =========================================================
#
# set.seed(123)
#
...
# y <- as.factor(y)
#
# # Transpose X only if needed
# # Rows must be samples and columns must be features
# X <- t(X)
#
# classes <- levels(y)
# k <- 10
...
# )
#
# RF_ovo_cv_storage <- list(
#   best_fold_train = list(),
#   best_fold_test = list(),
#   best_fold_xy = list(),
#   fold_models = list()
# )
...
# }
#
# safe_macro_metric <- function(cm_object, metric_name) {
#
#   by_class <- cm_object$byClass
#
#   if (is.null(dim(by_class))) {
#     return(as.numeric(by_class[metric_name]))
#   } else {
#     return(mean(by_class[, metric_name], na.rm = TRUE))
#   }
# }
#
# # =========================================================
# # 4. Cross-validation OvO
# # =========================================================
#
# for (fold_idx in seq_along(folds)) {
#
...
#
#   # ---------------------------------------------------------
#   # Storage for binary OvO models of this fold
#   # ---------------------------------------------------------
#
#   fold_models <- list()
#   fold_train_predictions <- list()
#   fold_test_predictions <- list()
#   fold_train_scores <- list()
#   fold_test_scores <- list()
#   fold_binary_confusion_train <- list()
#   fold_binary_confusion_test <- list()
#
#   # ---------------------------------------------------------
#   # Train one RF model for each OvO pair
#   # ---------------------------------------------------------
#
#   for (pair_idx in seq_along(class_pairs)) {
#
#     pair <- class_pairs[[pair_idx]]
#
#     class1 <- pair[1]
#     class2 <- pair[2]
#
#     pair_name <- paste(class1, "vs", class2)
#
#     cat("Training OvO model:", pair_name, "\n")
#
#     # Keep only training samples belonging to the current pair
#     pair_idx_train <- which(y_train %in% c(class1, class2))
#
#     pair_X_train <- X_train[pair_idx_train, , drop = FALSE]
#     pair_y_train <- droplevels(y_train[pair_idx_train])
#
#     # Train RF binary model
...
#     )
#
#     # Predict classes on full train and full test folds
#     pred_train <- predict(rf_model, X_train, type = "response")
#     pred_test <- predict(rf_model, X_test, type = "response")
#
#     # Predict probabilities/scores
#     prob_train <- predict(rf_model, X_train, type = "prob")
#     prob_test <- predict(rf_model, X_test, type = "prob")
#
#     # -------------------------------------------------------
#     # Update train votes
#     # -------------------------------------------------------
...
#
#     # -------------------------------------------------------
#     # Binary confusion matrices for the current pair
#     # Only samples belonging to this pair are evaluated
#     # -------------------------------------------------------
#
#     train_pair_eval_idx <- which(y_train %in% c(class1, class2))
#     test_pair_eval_idx <- which(y_test %in% c(class1, class2))
#
#     binary_cm_train <- table(
#       Predicted = pred_train[train_pair_eval_idx],
#       Actual = y_train[train_pair_eval_idx]
#     )
#
#     binary_cm_test <- table(
#       Predicted = pred_test[test_pair_eval_idx],
#       Actual = y_test[test_pair_eval_idx]
#     )
#
#     # -------------------------------------------------------
#     # Store binary model and predictions
#     # -------------------------------------------------------
#
#     fold_models[[pair_idx]] <- list(
#       pair = pair,
#       pair_name = pair_name,
#       model = rf_model,
#       pair_X_train = pair_X_train,
#       pair_y_train = pair_y_train
#     )
#
#     fold_train_predictions[[pair_idx]] <- pred_train
#     fold_test_predictions[[pair_idx]] <- pred_test
#
#     fold_train_scores[[pair_idx]] <- prob_train
#     fold_test_scores[[pair_idx]] <- prob_test
#
#     fold_binary_confusion_train[[pair_idx]] <- binary_cm_train
#     fold_binary_confusion_test[[pair_idx]] <- binary_cm_test
#   }
#
#   # ---------------------------------------------------------
...
#
#   y_pred_train <- factor(
#     apply(votes_train, 1, resolve_tie, classes = classes),
#     levels = classes
#   )
#
#   y_pred_test <- factor(
#     apply(votes_test, 1, resolve_tie, classes = classes),
#     levels = classes
#   )
#
#   # ---------------------------------------------------------
...
#   )
#
#   multi_confusion_train <- table(
#     Predicted = y_pred_train,
#     Actual = y_train
#   )
#
#   multi_confusion_test <- table(
...
#   # ---------------------------------------------------------
#
#   results_RF_ovo <- results_RF_ovo %>%
#     add_row(
#       Fold = fold_idx,
#
#       Accuracy.train = as.numeric(cm_train$overall["Accuracy"]),
#       Precision.train = safe_macro_metric(cm_train, "Precision"),
#       Recall.train = safe_macro_metric(cm_train, "Recall"),
#       F1.train = safe_macro_metric(cm_train, "F1"),
#
#       Accuracy.test = as.numeric(cm_test$overall["Accuracy"]),
#       Precision.test = safe_macro_metric(cm_test, "Precision"),
#       Recall.test = safe_macro_metric(cm_test, "Recall"),
#       F1.test = safe_macro_metric(cm_test, "F1")
#     )
#
#   # ---------------------------------------------------------
#   # Store everything needed for later analyses
#   # ---------------------------------------------------------
#
#   RF_ovo_cv_storage$best_fold_xy[[fold_idx]] <- list(
#     train_idx = train_idx,
#     test_idx = test_idx,
#     X_train = X_train,
#     X_test = X_test,
...
#   )
#
#   RF_ovo_cv_storage$fold_models[[fold_idx]] <- fold_models
#
#   RF_ovo_cv_storage$best_fold_train[[fold_idx]] <- list(
#     y_true = y_train,
#     y_pred = y_pred_train,
#     votes = votes_train,
#     confusionMatrix_object = cm_train,
#     Multi_confusionMatrix_ = multi_confusion_train,
#     confusionMatrix_ = fold_binary_confusion_train,
#     predictions = fold_train_predictions,
#     scores_predictions = fold_train_scores
#   )
#
#   RF_ovo_cv_storage$best_fold_test[[fold_idx]] <- list(
#     y_true = y_test,
#     y_pred = y_pred_test,
#     votes = votes_test,
#     confusionMatrix_object = cm_test,
#     Multi_confusionMatrix_ = multi_confusion_test,
#     confusionMatrix_ = fold_binary_confusion_test,
#     predictions = fold_test_predictions,
#     scores_predictions = fold_test_scores
#   )
# }
#
# # =========================================================
...
# # =========================================================
#
# print(results_RF_ovo)
#
# summary_RF_ovo <- results_RF_ovo %>%
#   summarise(
#     Mean_Accuracy_train = mean(Accuracy.train, na.rm = TRUE),
#     Mean_Precision_train = mean(Precision.train, na.rm = TRUE),
#     Mean_Recall_train = mean(Recall.train, na.rm = TRUE),
#     Mean_F1_train = mean(F1.train, na.rm = TRUE),
#
#     Mean_Accuracy_test = mean(Accuracy.test, na.rm = TRUE),
#     Mean_Precision_test = mean(Precision.test, na.rm = TRUE),
#     Mean_Recall_test = mean(Recall.test, na.rm = TRUE),
#     Mean_F1_test = mean(F1.test, na.rm = TRUE)
#   )
#
# print(summary_RF_ovo)
#
# # =========================================================
# # 6. Final object
# # =========================================================
#
# res_RF_ovo_2026 <- list(
#   method = "Random Forest OvO with Majority Voting",
#   classes = classes,
#   folds = folds,
#   class_pairs = class_pairs,
#   results = results_RF_ovo,
#   summary = summary_RF_ovo,
#   crossVal = RF_ovo_cv_storage
# )
# res_RF_ovo_imbalance_2026 = res_RF_ovo_2026
# # Optional save
# save(
#   res_RF_ovo_imbalance_2026,
#   file = "res_RF_ovo_imbalance_2026.rda"
# )
#
#
#
...
#
# # =========================================================
# # SVM OvO + MAJORITY VOTING WITH TIE-BREAKING
# # 10-fold cross-validation with full object storage
# # =========================================================
#
# set.seed(123)
...
#
#   # ---------------------------------------------------------
#   # Train one SVM model for each OvO pair
#   # ---------------------------------------------------------
#
#   for (pair_idx in seq_along(class_pairs)) {
#
...
#     pair_name <- paste(class1, "vs", class2)
#
#     cat("Training OvO SVM model:", pair_name, "\n")
#
#     # Keep only training samples belonging to the current pair
#     pair_idx_train <- which(y_train %in% c(class1, class2))
#
...
#     prob_train <- attr(pred_train, "probabilities")
#     prob_test <- attr(pred_test, "probabilities")
#
#     # -------------------------------------------------------
#     # Update train votes
#     # -------------------------------------------------------
...
#   # ---------------------------------------------------------
#
#   results_SVM_ovo <- results_SVM_ovo %>%
#     add_row(
#       Fold = fold_idx,
#
#       Accuracy.train = as.numeric(cm_train$overall["Accuracy"]),
...
#   # ---------------------------------------------------------
#
#   SVM_ovo_cv_storage$best_fold_xy[[fold_idx]] <- list(
#     train_idx = train_idx,
#     test_idx = test_idx,
#     X_train = X_train,
#     X_test = X_test,
...
#   )
#
#   SVM_ovo_cv_storage$fold_models[[fold_idx]] <- fold_models
#
#   SVM_ovo_cv_storage$best_fold_train[[fold_idx]] <- list(
#     y_true = y_train,
#     y_pred = y_pred_train,
#     votes = votes_train,
#     confusionMatrix_object = cm_train,
...
#   )
#
#   SVM_ovo_cv_storage$best_fold_test[[fold_idx]] <- list(
#     y_true = y_test,
#     y_pred = y_pred_test,
#     votes = votes_test,
#     confusionMatrix_object = cm_test,
...
# # =========================================================
#
# print(results_SVM_ovo)
#
# summary_SVM_ovo <- results_SVM_ovo %>%
#   summarise(
#     Mean_Accuracy_train = mean(Accuracy.train, na.rm = TRUE),
#     Mean_Precision_train = mean(Precision.train, na.rm = TRUE),
#     Mean_Recall_train = mean(Recall.train, na.rm = TRUE),
...
#   )
#
# print(summary_SVM_ovo)
#
# # =========================================================
# # 6. Final object
# # =========================================================
#
# res_SVM_ovo_2026 <- list(
#   method = "SVM OvO with Majority Voting",
#   kernel = "linear",
#   classes = classes,
#   folds = folds,
#   class_pairs = class_pairs,
...
#     preProc = preProc
#   ),
#   results = results_SVM_ovo,
#   summary = summary_SVM_ovo,
#   crossVal = SVM_ovo_cv_storage
# )
# res_SVM_ovo_T2D_2026 = res_SVM_ovo_2026
# # Optional save
# save(
#   res_SVM_ovo_T2D_2026,
#   file = "res_SVM_ovo_T2D_2026.rda"
# )
#
#
#
...
#
#     X_pair <- X_train[idx_pair_train, , drop = FALSE]
#     y_pair <- factor(y_train[idx_pair_train], levels = c(class1, class2))
#
#     # --- Model ---
#     model <- cv.glmnet(
#       as.matrix(X_pair),
...
#
#     # --- Binary confusion ---
#     idx_train_eval <- which(y_train %in% c(class1, class2))
#     idx_test_eval  <- which(y_test %in% c(class1, class2))
#
#     cm_train_bin <- table(
#       Predicted = pred_train[idx_train_eval],
#       Actual = y_train[idx_train_eval]
#     )
#
#     cm_test_bin <- table(
#       Predicted = pred_test[idx_test_eval],
#       Actual = y_test[idx_test_eval]
#     )
#
#     # --- Storage ---
#     fold_models[[pair_idx]] <- list(
...
#     fold_test_scores[[pair_idx]] <- prob_test
#
#     fold_binary_confusion_train[[pair_idx]] <- cm_train_bin
#     fold_binary_confusion_test[[pair_idx]] <- cm_test_bin
#   }
#
#   # --- Voting aggregation ---
#   y_pred_train <- factor(apply(votes_train, 1, resolve_tie, classes = classes), levels = classes)
...
#   cm_test  <- confusionMatrix(y_pred_test,  y_test)
#
#   multi_train <- table(Predicted = y_pred_train, Actual = y_train)
#   multi_test  <- table(Predicted = y_pred_test,  Actual = y_test)
#
#   # --- Metrics ---
#   results_LOGIT_ovo <- results_LOGIT_ovo %>%
#     add_row(
#       Fold = fold_idx,
#       Accuracy.train = cm_train$overall["Accuracy"],
#       Precision.train = safe_macro_metric(cm_train, "Precision"),
...
#
#   # --- Store everything ---
#   LOGIT_ovo_cv_storage$best_fold_xy[[fold_idx]] <- list(
#     X_train = X_train,
#     X_test = X_test,
#     y_train = y_train,
#     y_test = y_test
#   )
#
#   LOGIT_ovo_cv_storage$fold_models[[fold_idx]] <- fold_models
#
#   LOGIT_ovo_cv_storage$best_fold_train[[fold_idx]] <- list(
#     y_true = y_train,
#     y_pred = y_pred_train,
#     votes = votes_train,
#     Multi_confusionMatrix_ = multi_train,
#     confusionMatrix_ = fold_binary_confusion_train,
#     scores_predictions = fold_train_scores
#   )
#
#   LOGIT_ovo_cv_storage$best_fold_test[[fold_idx]] <- list(
#     y_true = y_test,
#     y_pred = y_pred_test,
#     votes = votes_test,
#     Multi_confusionMatrix_ = multi_test,
#     confusionMatrix_ = fold_binary_confusion_test,
#     scores_predictions = fold_test_scores
#   )
# }
...
# # =========================================================
#
# res_LOGIT_ovo_2026 <- list(
#   method = "Logistic Regression OvO",
#   classes = classes,
#   folds = folds,
#   class_pairs = class_pairs,
#   results = results_LOGIT_ovo,
#   crossVal = LOGIT_ovo_cv_storage
# )
#
# res_LOGIT_ovo_T2D_2026 = res_LOGIT_ovo_2026
#
# save(res_LOGIT_ovo_T2D_2026, file = "res_LOGIT_ovo_T2D_2026.rda")
#
# print(res_LOGIT_ovo_T2D_2026)
#
#
#
#
