...
################################################################
# @script: sota.RandomForest.R ovo
# @author: Edi Prifti
# @author: Fabien KAMBU
# @date: Octobre 2023
# @date: Octobre 2023 (a working integrated version with predomics)
################################################################


#' sota.rf: launching Random Forest classifier one versus one
#'
#' @title sota.rf_ovo
#' @importFrom randomForest randomForest
#' @description one versus one sota.rf is a wrapper that executes svm using the same framework as for the predomics package.
                    ...
                    experiment.save = "nothing")
{
  ...
  clf$learner                   <- "sota.rf_ovo" # name of the method
...
sota.rf_fit_ovo <- function(X, y, clf) {
  ...
  for(i in 1:length(clf$params$sparsity))  # sparsity is = k, i.e. the number of features in a model
  {
    ...
    for (o in 1:length(clf$feature.cor)){
      ...
      ### Launch randomForest with the parameters from the clf
      set.seed(clf$params$seed)
      rf <- randomForest(x = x.reduced, y=yi,
                         ntree=clf$params$ntree,
                         mtry=clf$params$mtry,
                         replace=clf$params$replace,
                         classwt=clf$params$classwt,
      ...
      collect_rf[[o]] <- rf
      list_selected_[[o]] <- selected.features
      list_indices[[o]] <- match(selected.features, rownames(X))
    }
    ...
    mod.res$learner          <- clf$learner
    mod.res$language         <- clf$params$language
    mod.res$names_           <- list_selected_
    # match the index
    mod.res$indices_         <- list_indices
    mod.res$eval.sparsity    <- length(unique(mod.res$names_[[1]]))

    # Add the rf object
    mod.res$obj              <- collect_rf
    mod.res$auc_             <- NA
    mod.res$cor_             <- NA
    mod.res$aic_             <- NA
    mod.res$score_           <- NA
